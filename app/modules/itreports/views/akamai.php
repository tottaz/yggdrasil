<script type="text/javascript" src="http://www.google.com/jsapi"></script>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Date', 'HTTP Total MB'],
          <?php foreach ($total_mb as $total_mbs) { ?>
          ['<?php echo $total_mbs['date']; ?>', <?php echo $total_mbs['http_total_mb']; ?>],
          <?php } ?>
          ]);
    ...
    <script type="text/javascript">
      ...
      function drawM95() {
        var data = google.visualization.arrayToDataTable([
          ['Date', '95/5 '],
          <?php foreach ($m95 as $m95s) { ?>
          ['<?php echo $m95s['date']; ?>', <?php echo $m95s['m95_mbps']; ?>],
          <?php } ?>
          ]);
    ...
    <script type="text/javascript">
      ...
      function drawTotalHits() {
        var data = google.visualization.arrayToDataTable([
          ['Date','Total Hits'],
          <?php foreach ($total_hits as $total_hitss) { ?>
          ['<?php echo $total_hitss['date']; ?>', <?php echo $total_hitss['total_hits']; ?>],
          <?php } ?>
          ]);
...
<h1 style='width:95%; padding:20px; font-family:Tahoma;font-weight:normal;background:#f2f3f4;'>Akamai Usage for torbjornzetterlund.com</h1>          
