...
class Grid_m extends My_Model {
		...
		function update($action, $data = 0, $skip_validation = false){
			...
			if ($this->validate($action)){ 
				$this->db->update("itdatas", $this, array("cdn_id" => $action->get_id()));
				$action->success();
			}
		}
		...
		function delete($action){
			$this->db->delete("itdatas", array("cdn_id" => $action->get_id()));
			$action->success();
		}
