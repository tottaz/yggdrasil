...
if ( isSet($_SERVER['SCRIPT_NAME']) && isSet( $_SERVER['REQUEST_URI']) ) {
    ...
    $scriptprefixlen = strpos($req_uri, $_SERVER['SCRIPT_NAME']);
    if ($scriptprefixlen === FALSE) {
        // SCRIPT_NAME is not part of the URI
        // search just the end portion if the SCRIPT_NAME ends with gateway.php[34]
        if (isphpfile($_SERVER['SCRIPT_NAME'])) {
            $scriptbase = end(explode('/', $_SERVER['SCRIPT_NAME']));
            $scriptprefixlen = strpos($req_uri, $scriptbase);
        }
    }

    $newscript = '';
    if ($scriptprefixlen === FALSE) {
        $uripath = explode('/', $req_uri);
        // SCRIPT_NAME or base of it is not part of the URI
        $nscript = '';
        foreach ($uripath as $fname) {
            if ($fname != '') {
                $nscript .= '/'.$fname;
                if (isphpfile($fname)) {
                    // gateway file found
    ...
    } elseif ($scriptprefixlen > 0) {
        // SCRIPT_NAME is not at the beginning part of the URI, adjust it
        $newscript = substr($req_uri, 0, $scriptprefixlen + strlen($scriptbase));
    }
