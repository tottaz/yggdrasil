...
if($merchant_id == '' || $link_id == '' || $link_to_go == '') {
  ...
  else { $linearray = split('&#036;',$base->input['subscribe']); }
  ...
  $extras = $rs->FetchNextObject(); 

  if ($extras->SEARCH_PATTERN != '') {
      $search = split('\$', $extras->SEARCH_PATTERN);
      $x = 0;
      $y = count($search);
      while ($x < $y) {
            if(!array_key_exists($search[$x], $base->input)) {
            ?>
                <script>
       		        alert('Merchant Link do not exist. <? echo $result ?>');
                    self.close();
                </script>   
            <?
            }
            $link_to_go .= "&" . $search[$x] ."=";
            $link_to_go .= $base->input[$search[$x]];
            $x++;
      }
            $_SESSION['link_to_go'] = $link_to_go; 
      }  
}

//
// Clean up any link that has expired.
//
...
?>
        <script>
       		alert('Merchant Link is not active. <? echo $result ?>');
            self.close();
         </script>   
<?
            exit;
...
//
    ...
    if($check_link != '') {
        ...
        if($check_link->TIME_DURATION > 0) {
            ...
            $dateStr = $check_link->DATE_LAST;
            $today = date( "Y-m-d", time() );

	        if($check_link->TIME_PERIOD == "W"){
		        $check_link->TIME_DURATION = $check_link->TIME_DURATION * 7;
	        }

	        $sqla  = "SELECT DATE_ADD('$dateStr', INTERVAL ".$check_link->TIME_DURATION." ".$acyclep[$check_link->TIME_PERIOD].") AS expire";
            $sqla .= " FROM ".TBL_MERCHANT_ACTIVE_LINK." WHERE id=".$check_link->ID;
            $rs = $zetadb->Execute($sqla);
		    $k = $rs->FetchNextObject();
           
            if(dpdate4($k->EXPIRE) <= $today ) {
                $zetadb->Execute("DELETE FROM ".TBL_MERCHANT_ACTIVE_LINK." WHERE link_id='$link_id' AND id='$user' AND link_to_go='$link_to_go'");
            }             
        }

        $rs = $zetadb->Execute("SELECT * FROM ".TBL_MERCHANT_ACTIVE_LINK." WHERE merchant_id=$merchant_id AND id='$user' AND link_to_go='$link_to_go'");
        $link_exist = $rs->FetchNextObject();
        
        if($link_exist != '' AND $link_exist->DURATION > '0') {
                //
                ...
                //
                //  Prepare the return link - create a secret to pass on back
                //
	            $merchant_link = $a->URL_LINK;
                $merchant_link .= str_replace(".html", "-", $link_to_go);
            
                $secret = $a->SECRET;

                $hash = md5(implode(':', array($secret, $link_to_go)));
                $hash = md5($secret.$hash);            

                if ($a->SEARCH_PATTERN != '') {
                    $merchant_link .= '&bqhash=' .$hash;
                } else {
                    $merchant_link .= $hash . ".html";
                }
...
//            issue_pass($host, $gateway, $path, $qstr, $passexp, $secret);
            ...
            //  Still Duration left on the link, subtract duration, and return to the merchant link
            //
            $zetadb->Execute("UPDATE ".TBL_MERCHANT_ACTIVE_LINK." SET duration=duration-1, hash='$hash' WHERE merchant_id='$merchant_id' AND link_id='$link_id' AND id='$user' AND link_to_go='$link_to_go'");
...
//      	            location.href('https://www.banqpay.com/sublink.php');   
                </script>   
                <?
	        }
...
//
      transact($user,$a->MERCHANT_ID,($a->PRICE),'Purchase','',0, 0,'',addslashes($a->LINK_NAME));
//
...
//
	...
	$zetadb->Execute("INSERT INTO ".TBL_MERCHANT_ACTIVE_LINK." SET link_id='$link_id', merchant_id='$a->MERCHANT_ID' ,id='$user', duration='$a->DURATION', time_duration='$a->TIME_DURATION', time_period='$a->TIME_PERIOD', link_to_go='$link_to_go', date_last='$expirydate'");
//
//  Prepare the return link - create a secret to pass on back
//
	        $merchant_link = $a->URL_LINK;
...
<DIV id=banner>
<TABLE width=570 height=60 border=0 cellspacing=0 cellpadding=0>
 <TR>
  <TD width=10></TD>
...
<IMG src='zetapay/images/buyer/rur.png' border=0 alt='image' height=10 width=10 style='position:absolute; top:0px; left:540px;'>
<IMG src='zetapay/images/buyer/rlr.png' border=0 alt='image' height=10 width=10 style='position:absolute; bottom:0px; left:540px;'>
<CENTER>
...
<TABLE width=500 border=0 cellspacing=0 cellpadding=0>
 ...
 <TR>
  <TD height=18 align=right>Email address&nbsp;</TD>
  <TD colspan=2>
 ...
 <TR>
  <TD colspan=3 height=10 align=right valign=bottom>
   <IMG src='zetapay/images/buyer/signin.png' alt='Login' border=0 width=78 height=34 onClick='submit0();' style='cursor:pointer;'>
   <IMG src='zetapay/images/buyer/createaccnt.png' alt='Create an account' border=0 width=144 height=44 onClick='signup();' style='position:absolute; bottom:15px; right:400px; cursor:pointer;'>
   <P class=infoText><br><br><B>Easy.</B> Quick to set up. Pay as little as one cent with a single click.</P>
...
<TABLE width=500 border=0 cellspacing=0 cellpadding=0 style='padding:0px; margin:0px;'>
 <TR>
...
if($a->TIME_PERIOD == 's') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Second</SPAN>
<?

} elseif($a->TIME_PERIOD == 'm') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Minute</SPAN>
<?

} elseif($a->TIME_PERIOD == 'h') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Hour</SPAN>
<?

} elseif($a->TIME_PERIOD == 'D') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Day</SPAN>
<?
} elseif($a->TIME_PERIOD == 'W') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Week</SPAN>
<?
} elseif($a->TIME_PERIOD == 'M') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Months</SPAN>
<?
} elseif($a->TIME_PERIOD == 'Y') {
?>
   <SPAN style='font-size:10px;'>Up to <? echo $a->DURATION ?> visits within <? echo $a->TIME_DURATION ?> Year</SPAN>
<?
...
<a href=sublink.php?cmd=logout><IMG src='zetapay/images/buyer/signout.png' alt='Sign out' border=0 width=78 height=34 style='position:absolute; bottom:10px; right:88px; cursor:pointer;'></a>
...
<TABLE width=515 border=0 cellspacing=0 cellpadding=0 border:1px style='background-color:#003399;'>
 <TR>
...
<TABLE width=515 border=0 cellspacing=0 cellpadding=0 style='background-color:#003399; background-position:top; background-repeat:repeat-x; border:1px solid #7BAF51; margin-top:6px;'>
 <TR>
...
?>
 ...
 <TR>
  ...
  <TD></TD>
  <TD colspan=2>
   <SPAN id='ccard_CVV2C' style='font-size:10px; font-weight:normal;'>Card verification number</SPAN> [<A href='javascript:ccard_cvv2help();'>?</A>]
  </TD>
