...
$create_account_remote = $_SESSION['create_account_remote'];
...
if($merchant_id == '' || $link_id == '' || $link_to_go == '') {
  ...
  $_SESSION['cycle'] = $linearray[4];
  $_SESSION['create_account_remote'] = $linearray[5];
...
//
if($link_id != '') {
    $rs = $zetadb->Execute("SELECT * FROM ".TBL_MERCHANT_SUBSCRIPTION_LINK." WHERE merchant_id='$merchant_id' and subscription_type='$subtype' and cycle_p='$cycle'");
...
?>
        <script>
       		alert('Merchant Subscription Link do not exist. <? echo $result ?>');
            self.close();
         </script>   
<?
        exit;
    } else {
        $rs = $zetadb->Execute("SELECT * FROM ".TBL_MERCHANT_SUBSCRIPTION_LINK." WHERE merchant_id='$merchant_id' and subscription_type='$subtype' and cycle_p='$cycle' AND active='Y'");
        $check_link = $rs->FetchNextObject();
        if($check_link == '') {
...
?>
        <script>
       		alert('Merchant Subscription Link is not active. <? echo $result ?>');
            self.close();
         </script>   
<?
            exit;
    ...
    if (!$errlogin) {
        ...
        //
        $zetadb->Execute("UPDATE ".TBL_MERCHANT_SUBSCRIPTION_LINK." SET clicks=clicks+1 WHERE merchant_id='$merchant_id' and subscription_type='$subtype' and cycle_p='$cycle'");
    }
}
...
//      <script>
//        javascript:popwindow('http://www.banqpay.com/subscribelink.php?$105$11$','scrollbars,resizable,top=15,left=15,width=620,height=360')
//    </script>
//
    $suid = $_SESSION['suid'];
...
//       	            location.href('http://www.banqpay.com/subscribelink.php');   
                </script>   
                <?
	        }
...
//
//              Add link to subscriber profile
//
                $zetadb->Execute("INSERT INTO ".TBL_USER_SUBSCRIPTIONS." SET user='$user', merchant_id='$a->MERCHANT_ID', setup='$a->SETUP', amount='$a->PRICE', memo='$a->MEMO', cycle_d='$a->CYCLE_D', cycle_p='$a->CYCLE_P', stop='$a->STOP', ticks='1', created=NOW(), subscription_type='$a->SUBSCRIPTION_TYPE', currency_code='$a->CURRENCY_CODE'");
                
//
//  Deduct purchase price from merchant account
//
                transact($user,$a->MERCHANT_ID,($a->PRICE),'Subscription','',0, 0,'',addslashes($a->ITEM_NAME));
                
                if ($create_account_remote == 1) {

                    $merchant_link = $a->RETURN_URL;
                    $merchant_link .= str_replace(".html", "-", $link_to_go);
//                    $secret = $a->SECRET;

//                    $hashuser = md5(implode(':', array($secret, $data->USERNAME)));
//                    $hashuser = md5($secret.$hashuser);            

//                    $hashpassw = md5(implode(':', array($secret, $data->PASSWORD)));
//                  $hashpassw = md5($secret.$hashpasswd);            
                    
//                  $merchant_link .= $hashuser . "-" . $hashpassw . ".html";
                  $merchant_link .= $data->USERNAME . "-" . $data->PASSWORD . ".html";
                ...
                unset($_SESSION['create_account_remote']);
...
<FORM id=f0 action='subscribelink.php' method=post>
...
<FORM id=f1 name=f action='subscribelink.php' method=post>
...
?>
<SCRIPT language=javascript type='text/javascript'>
		alert('The subscription link your try to access, do not exist.');
</SCRIPT>
<?
}
?>
...
<FORM id='auth_f0' action='subscribelink.php' method=post target='_self'>
...
if($a->CYCLE_P == 'D') {
?>
   <SPAN style='font-size:10px;'>You have selected to Subscribe for <? echo $a->CYCLE_D ?> Day</SPAN>
<?

} elseif($a->CYCLE_P == 'W') {
?>
   <SPAN style='font-size:10px;'>You have selected to Subscribe for <? echo $a->CYCLE_D ?> Week</SPAN>
<?

} elseif($a->CYCLE_P == 'M') {
?>
   <SPAN style='font-size:10px;'>You have selected to Subscribe for <? echo $a->CYCLE_D ?> Month</SPAN>
<?

} elseif($a->CYCLE_P == 'Y') {
?>
   <SPAN style='font-size:10px;'>You have selected to Subscribe for <? echo $a->CYCLE_D ?> Year</SPAN>
<?
...
<a href=subscribelink.php?cmd=logout><IMG src='zetapay/images/buyer/signout.png' alt='Sign out' border=0 width=78 height=34 style='position:absolute; bottom:10px; right:88px; cursor:pointer;'></a>
...
<FORM id='gift_f0' action='subscribelink.php' method=post target='BQ_TEST'>
...
<TR>
 ...
 <TD valign=top>
  ...
  <FORM id='fund_f0' action='subscribelink.php' method=post target='BQ_TEST'>
  ...
  <FORM id='fund_f1' action='subscribelink.php' method=post>
...
<FORM id='ccard_f0' action='subscribelink.php' method=post>
