...
// ADDED NEW FOR WORD WRAPWWWW
function FCell($w,$h,$txt,$border=0,$align='J',$fill=0,$maxline=0,$wwrap='') 
{ 
...
if($c==' '){ 
	ECHO "\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\HERE";
$sep=$i; 
$ls=$l; 
$ns++; 
} 
...
{ 
//Automatic line break 
ECHO "\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\jack";
...
return substr($s,$i); 
	...
	{
	ECHO "\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\N\NPACK";
$i++;
	}
} 
//Last chunk 
...
{		
	...
	{
		$line[$i]=$this->NbLines($wid[$i],$data[$i]);
        $nb=max($nb,$this->NbLines($wid[$i],$data[$i]));
	}
    $h=4*$nb;
    ...
    {		
	  ...
	  if(count($align)<1)
        ...
        $this->Rect($x,$y,$w,$h);
        //Print the text
        $this->MultiCell($w,$height,$data[$i],1,$a,1);
		 //$this->MultiCell($w,$height," ",1,$a,1);
...
function NoticeRow($data,$wid,$align="")
{		
	$this->SetTextColor(black);
	$this->SetDrawColor(black);
	$this->SetLineWidth(.3);
    ...
    {		
	  ...
	  if(count($align)<1)
        ...
        //Print the text
        if($i!=2)
			$this->MultiCell($w,$height,$data[$i],0,$a,1);
		else
 		   $this->MultiCell($w,$height,$data[$i],1,$a,1);
		 //$this->MultiCell($w,$height," ",1,$a,1);
        //Put the position to the right of the cell
...
function NoticeRow_WT_Heading($data,$wid,$align="")
{		
	$this->SetTextColor(black);
	$this->SetDrawColor(black);
	$this->SetLineWidth(.3);
...
function RowTeamPerform($data,$wid,$align="")
{	
...
function RowAttribute($data,$wid,$align="")
{	
...
function RowSubAttribute($data,$wid,$align="")
{	
    ...
    for($i=0;$i<count($data);$i++)
    {		
		if($data[$i]>90  || $data[$i]==0 || strlen($data[$i])>7)
		{
			$this->SetTextColor(black);
		}
		else if($data[$i]<90 && (strlen($data[$i])>0 || strlen($data[$i])<7))
		{
			$this->SetTextColor(255,0,0);
		}
      $w=$wid[$i];
