...
* @constructor
*/
BANQPAY.widget.Tooltip = function(el, userConfig) {
	if (arguments.length > 0) {
		BANQPAY.widget.Tooltip.superclass.constructor.call(this, el, userConfig);
	}
}

BANQPAY.widget.Tooltip.prototype = new BANQPAY.widget.Overlay();
BANQPAY.widget.Tooltip.prototype.constructor = BANQPAY.widget.Tooltip;
...
* @final
*/
BANQPAY.widget.Tooltip.superclass = BANQPAY.widget.Overlay.prototype;
...
* @final
*/
BANQPAY.widget.Tooltip.CSS_TOOLTIP = "tt";
...
*/
BANQPAY.widget.Tooltip.prototype.init = function(el, userConfig) {
	if (document.readyState && document.readyState != "complete") {
		var deferredInit = function() {
			this.init(el, userConfig);
		}
		BANQPAY.util.Event.addListener(window, "load", deferredInit, this, true);
	} else {
		BANQPAY.widget.Tooltip.superclass.init.call(this, el);

		this.beforeInitEvent.fire(BANQPAY.widget.Tooltip);

		BANQPAY.util.Dom.addClass(this.element, BANQPAY.widget.Tooltip.CSS_TOOLTIP);
		...
		this.initEvent.fire(BANQPAY.widget.Tooltip);
	}
}
...
BANQPAY.widget.Tooltip.prototype.initDefaultConfig = function() {
	BANQPAY.widget.Tooltip.superclass.initDefaultConfig.call(this);
...
*/
BANQPAY.widget.Tooltip.prototype.configText = function(type, args, obj) {
	var text = args[0];
	if (text) {
		this.setBody(text);
	}
...
*/
BANQPAY.widget.Tooltip.prototype.configContainer = function(type, args, obj) {
...
*/
BANQPAY.widget.Tooltip.prototype.configPreventOverlap = function(type, args, obj) {
	var preventoverlap = args[0];
	if (preventoverlap) {
		if (! BANQPAY.util.Config.alreadySubscribed(this.moveEvent, this.preventOverlap, this)) {
...
* The default event handler fired when the "context" property is changed.
*/
BANQPAY.widget.Tooltip.prototype.configContext = function(type, args, obj) {
	var context = args[0];
	if (context) {
		...
		BANQPAY.util.Event.addListener(contextElement, "mouseover", this.onContextMouseOver, this);
		BANQPAY.util.Event.addListener(contextElement, "mouseout", this.onContextMouseOut, this);
	}
}
...
*/
BANQPAY.widget.Tooltip.prototype.onContextMouseOver = function(e, obj) {
	if (! obj) {
		obj = this;
	}
...
* @param {object}	obj	The object argument
*/
BANQPAY.widget.Tooltip.prototype.onContextMouseOut = function(e, obj) {
	if (! obj) {
		obj = this;
	}
...
*/
BANQPAY.widget.Tooltip.prototype.doShow = function(e) {

	var pageX = BANQPAY.util.Event.getPageX(e);
	var pageY = BANQPAY.util.Event.getPageY(e);
...
*/
BANQPAY.widget.Tooltip.prototype.doHide = function() {
	var me = this;
	setTimeout(
		function() {
			me.hide();
...
*/
BANQPAY.widget.Tooltip.prototype.preventOverlap = function(type, args, obj) {
	var pos = args[0];
	var x = pos[0];
	var y = pos[1];

	var elementRegion = BANQPAY.util.Dom.getRegion(this.element);
	var contextRegion = BANQPAY.util.Dom.getRegion(this.cfg.getProperty("context"));
...
* @constructor
*/
BANQPAY.widget.Panel = function(el, userConfig) {
	if (arguments.length > 0) {
		BANQPAY.widget.Panel.superclass.constructor.call(this, el, userConfig);
	}
}

BANQPAY.widget.Panel.prototype = new BANQPAY.widget.Overlay();
BANQPAY.widget.Panel.prototype.constructor = BANQPAY.widget.Panel;
...
* @final
*/
BANQPAY.widget.Panel.superclass = BANQPAY.widget.Overlay.prototype;
...
* @final
*/
BANQPAY.widget.Panel.CSS_PANEL = "panel";
...
* @final
*/
BANQPAY.widget.Panel.CSS_PANEL_CONTAINER = "panel-container";
...
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Panel.prototype.showMaskEvent = null;
...
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Panel.prototype.hideMaskEvent = null;
...
* @param {object}	userConfig	The configuration object literal containing the configuration that should be set for this Overlay. See configuration documentation for more details.
*/
BANQPAY.widget.Panel.prototype.init = function(el, userConfig) {
	BANQPAY.widget.Panel.superclass.init.call(this, el/*, userConfig*/);  // Note that we don't pass the user config in here yet because we only want it executed once, at the lowest subclass level
	
	this.beforeInitEvent.fire(BANQPAY.widget.Panel);

	BANQPAY.util.Dom.addClass(this.element, BANQPAY.widget.Panel.CSS_PANEL);
	...
	this.initEvent.fire(BANQPAY.widget.Panel);
...
* Initializes the custom events for Module which are fired automatically at appropriate times by the Module class.
*/
BANQPAY.widget.Panel.prototype.initEvents = function() {
	BANQPAY.widget.Panel.superclass.initEvents.call(this);

	this.showMaskEvent = new BANQPAY.util.CustomEvent("showMask");
	this.hideMaskEvent = new BANQPAY.util.CustomEvent("hideMask");
}
...
BANQPAY.widget.Panel.prototype.initDefaultConfig = function() {
	BANQPAY.widget.Panel.superclass.initDefaultConfig.call(this);
...
*/
BANQPAY.widget.Panel.prototype.configClose = function(type, args, obj) {
	var val = args[0];
	...
	if (val) {
		if (! this.close) {
			...
			BANQPAY.util.Dom.addClass(this.close, "close");

			if (this.isSecure) {
				BANQPAY.util.Dom.addClass(this.close, "secure");
			} else {
				BANQPAY.util.Dom.addClass(this.close, "nonsecure");
			}

			this.close.innerHTML = "&nbsp;";
			this.innerElement.appendChild(this.close);
			BANQPAY.util.Event.addListener(this.close, "click", doHide, this);	
		} else {
...
*/
BANQPAY.widget.Panel.prototype.configDraggable = function(type, args, obj) {
	var val = args[0];
	if (val) {
		if (this.header) {
			BANQPAY.util.Dom.setStyle(this.header,"cursor","move");
		...
		if (this.header) {
			BANQPAY.util.Dom.setStyle(this.header,"cursor","auto");
		}
	}
}
...
*/
BANQPAY.widget.Panel.prototype.configUnderlay = function(type, args, obj) {
	var val = args[0];

	switch (val.toLowerCase()) {
		case "shadow":
			BANQPAY.util.Dom.removeClass(this.element, "matte");
			BANQPAY.util.Dom.addClass(this.element, "shadow");
			...
			break;
		case "matte":
			BANQPAY.util.Dom.removeClass(this.element, "shadow");
			BANQPAY.util.Dom.addClass(this.element, "matte");
			break;
		case "none":
		default:
			BANQPAY.util.Dom.removeClass(this.element, "shadow");
...
*/
BANQPAY.widget.Panel.prototype.configModal = function(type, args, obj) {
	var modal = args[0];

	if (modal) {
		this.buildMask();

		if (! BANQPAY.util.Config.alreadySubscribed( this.showEvent, this.showMask, this ) ) {
			this.showEvent.subscribe(this.showMask, this, true);
		}
		if (! BANQPAY.util.Config.alreadySubscribed( this.hideEvent, this.hideMask, this) ) {
			this.hideEvent.subscribe(this.hideMask, this, true);
		}
		if (! BANQPAY.util.Config.alreadySubscribed( BANQPAY.widget.Overlay.windowResizeEvent, this.sizeMask, this ) ) {
			BANQPAY.widget.Overlay.windowResizeEvent.subscribe(this.sizeMask, this, true);
		}
	} else {
...
BANQPAY.widget.Panel.prototype.configKeyListeners = function(type, args, obj) {
	...
	if (listeners) {
		if (listeners instanceof Array) {
			for (var i=0;i<listeners.length;i++) {
				...
				if (! BANQPAY.util.Config.alreadySubscribed(this.showEvent, listener.enable, listener)) {
					this.showEvent.subscribe(listener.enable, listener, true);
				}
				if (! BANQPAY.util.Config.alreadySubscribed(this.hideEvent, listener.disable, listener)) {
					this.hideEvent.subscribe(listener.disable, listener, true);
				}
			}
		} else {
			if (! BANQPAY.util.Config.alreadySubscribed(this.showEvent, listeners.enable, listeners)) {
				this.showEvent.subscribe(listeners.enable, listeners, true);
			}
			if (! BANQPAY.util.Config.alreadySubscribed(this.hideEvent, listeners.disable, listeners)) {
...
*/
BANQPAY.widget.Panel.prototype.buildWrapper = function() {
	...
	BANQPAY.util.Dom.addClass(this.innerElement, "panel");

	var wrapper = document.createElement("DIV");
	wrapper.className = BANQPAY.widget.Panel.CSS_PANEL_CONTAINER;
...
*/
BANQPAY.widget.Panel.prototype.sizeUnderlay = function() {
...
* Event handler fired when the resize monitor element is resized.
*/
BANQPAY.widget.Panel.prototype.onDomResize = function(e, obj) { 
	BANQPAY.widget.Panel.superclass.onDomResize.call(this, e, obj);
	var me = this;
	setTimeout(function() {
		me.sizeUnderlay();
	}, 0);
...
*/
BANQPAY.widget.Panel.prototype.registerDragDrop = function() {
	if (this.header) {
		this.dd = new BANQPAY.util.DD(this.element.id, "panel");
		...
		this.dd.startDrag = function() {
			if (me.browser == "ie") {
				BANQPAY.util.Dom.addClass(me.element,"drag");
			}
		...
		this.dd.endDrag = function() {
			if (me.browser == "ie") {
				BANQPAY.util.Dom.removeClass(me.element,"drag");
			}
		}
...
*/
BANQPAY.widget.Panel.prototype.buildMask = function() {
	if (! this.mask) {
		...
		var maskClick = function(e, obj) {
			BANQPAY.util.Event.stopEvent(e);
		}

		BANQPAY.util.Event.addListener(this.mask, maskClick, this);
...
*/
BANQPAY.widget.Panel.prototype.hideMask = function() {
	if (this.cfg.getProperty("modal") && this.mask) {
		this.mask.tabIndex = -1;
		this.mask.style.display = "none";
		this.hideMaskEvent.fire();
		BANQPAY.util.Dom.removeClass(document.body, "masked");
	}
}
...
*/
BANQPAY.widget.Panel.prototype.showMask = function() {
	if (this.cfg.getProperty("modal") && this.mask) {
		BANQPAY.util.Dom.addClass(document.body, "masked");
		this.sizeMask();
...
*/
BANQPAY.widget.Panel.prototype.sizeMask = function() {
	if (this.mask) {
		this.mask.style.height = BANQPAY.util.Dom.getDocumentHeight()+"px";
		this.mask.style.width = BANQPAY.util.Dom.getDocumentWidth()+"px";
	}
}
...
* The default event handler fired when the "height" property is changed.
*/
BANQPAY.widget.Panel.prototype.configHeight = function(type, args, obj) {
	var height = args[0];
...
* The default event handler fired when the "width" property is changed.
*/
BANQPAY.widget.Panel.prototype.configWidth = function(type, args, obj) {
	var width = args[0];
	var el = this.innerElement;
	BANQPAY.util.Dom.setStyle(el, "width", width);
	this.cfg.refireEvent("underlay");
...
* @return {boolean} Success or failure of the render
*/
BANQPAY.widget.Panel.prototype.render = function(appendToNode) {
	return BANQPAY.widget.Panel.superclass.render.call(this, appendToNode, this.innerElement);
}
/**
Copyright (c) 2006, BANQPAY! Inc. All rights reserved.
Code licensed under the BSD License:
...
* @constructor
*/
BANQPAY.widget.Dialog = function(el, userConfig) {
	if (arguments.length > 0) {
		BANQPAY.widget.Dialog.superclass.constructor.call(this, el, userConfig);
	}
}

BANQPAY.widget.Dialog.prototype = new BANQPAY.widget.Panel();
BANQPAY.widget.Dialog.prototype.constructor = BANQPAY.widget.Dialog;
...
* @final
*/
BANQPAY.widget.Dialog.superclass = BANQPAY.widget.Panel.prototype;
...
* @final
*/
BANQPAY.widget.Dialog.CSS_DIALOG = "dialog";
...
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.beforeSubmitEvent = null;

/**
* CustomEvent fired after submission
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.submitEvent = null;

/**
* CustomEvent fired prior to manual submission
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.manualSubmitEvent = null;

/**
* CustomEvent fired prior to asynchronous submission
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.asyncSubmitEvent = null;

/**
* CustomEvent fired prior to form-based submission
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.formSubmitEvent = null;

/**
* CustomEvent fired after cancel
* @type BANQPAY.util.CustomEvent
*/
BANQPAY.widget.Dialog.prototype.cancelEvent = null;


/**
* Initializes the class's configurable properties which can be changed using the Dialog's Config object (cfg).
*/
BANQPAY.widget.Dialog.prototype.initDefaultConfig = function() {
	BANQPAY.widget.Dialog.superclass.initDefaultConfig.call(this);
	...
	*/
	...
	this.doSubmit = function() {
		...
		switch (method) {
			case "async":
				BANQPAY.util.Connect.setForm(this.form.name);
				var cObj = BANQPAY.util.Connect.asyncRequest('POST', this.form.action, callback);
...
*/
BANQPAY.widget.Dialog.prototype.initEvents = function() {
	BANQPAY.widget.Dialog.superclass.initEvents.call(this);
	
	this.beforeSubmitEvent	= new BANQPAY.util.CustomEvent("beforeSubmit");
	this.submitEvent		= new BANQPAY.util.CustomEvent("submit");

	this.manualSubmitEvent	= new BANQPAY.util.CustomEvent("manualSubmit");
	this.asyncSubmitEvent	= new BANQPAY.util.CustomEvent("asyncSubmit");
	this.formSubmitEvent	= new BANQPAY.util.CustomEvent("formSubmit");

	this.cancelEvent		= new BANQPAY.util.CustomEvent("cancel");
}
...
* @param {object}	userConfig	The configuration object literal containing the configuration that should be set for this Dialog. See configuration documentation for more details.
*/
BANQPAY.widget.Dialog.prototype.init = function(el, userConfig) {
	BANQPAY.widget.Dialog.superclass.init.call(this, el/*, userConfig*/);  // Note that we don't pass the user config in here yet because we only want it executed once, at the lowest subclass level
	
	this.beforeInitEvent.fire(BANQPAY.widget.Dialog);

	BANQPAY.util.Dom.addClass(this.element, BANQPAY.widget.Dialog.CSS_DIALOG);
	...
	this.initEvent.fire(BANQPAY.widget.Dialog);
}

/**
* Prepares the Dialog's internal FORM object, creating one if one is not currently present.
*/
BANQPAY.widget.Dialog.prototype.registerForm = function() {
	...
	if (this.cfg.getProperty("modal") && this.form) {
		...
		if (firstElement) {
			this.preventBackTab = new BANQPAY.util.KeyListener(firstElement, { shift:true, keys:9 }, {fn:me.focusLast, scope:me, correctScope:true} );
		...
		if (lastElement) {
			this.preventTabOut = new BANQPAY.util.KeyListener(lastElement, { shift:false, keys:9 }, {fn:me.focusFirst, scope:me, correctScope:true} );
...
*/
BANQPAY.widget.Dialog.prototype.configButtons = function(type, args, obj) {
	var buttons = args[0];
	if (buttons != "none") {
		...
		for (var b=0;b<buttons.length;b++) {
			...
			BANQPAY.util.Event.addListener(htmlButton, "click", button.handler, this, true);
...
*/
BANQPAY.widget.Dialog.prototype.focusLast = function(type,args,obj) {
	if (args) {
		var e = args[1];
		if (e) {
			BANQPAY.util.Event.stopEvent(e);
...
*/
BANQPAY.widget.Dialog.prototype.focusDefaultButton = function() {
...
*/
BANQPAY.widget.Dialog.prototype.blurButtons = function() {
	var buttons = this.cfg.getProperty("buttons");
	if (buttons && buttons instanceof Array) {
...
*/
BANQPAY.widget.Dialog.prototype.focusFirstButton = function() {
	var buttons = this.cfg.getProperty("buttons");
	if (buttons && buttons instanceof Array) {
		var html = buttons[0].htmlButton;
		if (html) {
...
* Sets the focus to the first button in the button list
*/
BANQPAY.widget.Dialog.prototype.focusLastButton = function() {
	var buttons = this.cfg.getProperty("buttons");
	if (buttons && buttons instanceof Array) {
...
*/
BANQPAY.widget.Dialog.prototype.validate = function() {
	return true;
}
...
*/
BANQPAY.widget.Dialog.prototype.submit = function() {
	if (this.validate()) {
...
*/
BANQPAY.widget.Dialog.prototype.cancel = function() {
...
*/
BANQPAY.widget.Dialog.prototype.getData = function() {
	var form = this.form;
	var data = {};
...
* @constructor
*/
BANQPAY.widget.SimpleDialog = function(el, userConfig) {
	if (arguments.length > 0) {
		BANQPAY.widget.SimpleDialog.superclass.constructor.call(this, el, userConfig);
	}
}

BANQPAY.widget.SimpleDialog.prototype = new BANQPAY.widget.Dialog();
BANQPAY.widget.SimpleDialog.prototype.constructor = BANQPAY.widget.SimpleDialog;
...
* @final
*/
BANQPAY.widget.SimpleDialog.superclass = BANQPAY.widget.Dialog.prototype;
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_BLOCK = "nt/ic/ut/bsc/blck16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_ALARM = "nt/ic/ut/bsc/alrt16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_HELP  = "nt/ic/ut/bsc/hlp16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_INFO  = "nt/ic/ut/bsc/info16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_WARN  = "nt/ic/ut/bsc/warn16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.ICON_TIP   = "nt/ic/ut/bsc/tip16_1.gif";
...
* @final
*/
BANQPAY.widget.SimpleDialog.CSS_SIMPLEDIALOG = "simple-dialog";
...
BANQPAY.widget.SimpleDialog.prototype.initDefaultConfig = function() {
	BANQPAY.widget.SimpleDialog.superclass.initDefaultConfig.call(this);
...
* @param {object}	userConfig	The configuration object literal containing the configuration that should be set for this SimpleDialog. See configuration documentation for more details.
*/
BANQPAY.widget.SimpleDialog.prototype.init = function(el, userConfig) {
	BANQPAY.widget.SimpleDialog.superclass.init.call(this, el/*, userConfig*/);  // Note that we don't pass the user config in here yet because we only want it executed once, at the lowest subclass level

	this.beforeInitEvent.fire(BANQPAY.widget.SimpleDialog);

	BANQPAY.util.Dom.addClass(this.element, BANQPAY.widget.SimpleDialog.CSS_SIMPLEDIALOG);
	...
	this.initEvent.fire(BANQPAY.widget.SimpleDialog);
...
*/
BANQPAY.widget.SimpleDialog.prototype.registerForm = function() {
	BANQPAY.widget.SimpleDialog.superclass.registerForm.call(this);
...
*/
BANQPAY.widget.SimpleDialog.prototype.configIcon = function(type,args,obj) {
	var icon = args[0];
	if (icon && icon != "none") {
...
*/
BANQPAY.widget.SimpleDialog.prototype.configText = function(type,args,obj) {
	var text = args[0];
	if (text) {
		this.setBody(text);
	}
...
* @param {object}	attrIn		The object literal representing the animation arguments to be used for the animate-in transition. The arguments for this literal are: attributes(object, see BANQPAY.util.Anim for description), duration(float), and method(i.e. BANQPAY.util.Easing.easeIn).
* @param {object}	attrOut		The object literal representing the animation arguments to be used for the animate-out transition. The arguments for this literal are: attributes(object, see BANQPAY.util.Anim for description), duration(float), and method(i.e. BANQPAY.util.Easing.easeIn).
...
BANQPAY.widget.ContainerEffect = function(overlay, attrIn, attrOut, targetElement) {
	...
	this.beforeAnimateInEvent = new BANQPAY.util.CustomEvent("beforeAnimateIn");
	this.beforeAnimateOutEvent = new BANQPAY.util.CustomEvent("beforeAnimateOut");

	this.animateInCompleteEvent = new BANQPAY.util.CustomEvent("animateInComplete");
	this.animateOutCompleteEvent = new BANQPAY.util.CustomEvent("animateOutComplete");
}

/**
* Initializes the animation classes and events.
* @param {class}	Optional. The animation class to instantiate. Defaults to BANQPAY.util.Anim. Other options include BANQPAY.util.Motion.
*/
BANQPAY.widget.ContainerEffect.prototype.init = function(animClass) {
	if (! animClass) {
		animClass = BANQPAY.util.Anim;
	}
...
*/
BANQPAY.widget.ContainerEffect.prototype.animateIn = function() {
...
*/
BANQPAY.widget.ContainerEffect.prototype.animateOut = function() {
...
*/
BANQPAY.widget.ContainerEffect.FADE = function(overlay, dur) {
	var fade = new BANQPAY.widget.ContainerEffect(overlay, { attributes:{opacity: {from:0, to:1}}, duration:dur, method:BANQPAY.util.Easing.easeIn }, { attributes:{opacity: {to:0}}, duration:dur, method:BANQPAY.util.Easing.easeOut} );

	fade.handleStartAnimateIn = function(type,args,obj) {
		BANQPAY.util.Dom.addClass(obj.overlay.element, "hide-select");
		...
		if (obj.overlay.underlay) {
			obj.initialUnderlayOpacity = BANQPAY.util.Dom.getStyle(obj.overlay.underlay, "opacity");
			obj.overlay.underlay.style.filter = null;
		}

		BANQPAY.util.Dom.setStyle(obj.overlay.element, "visibility", "visible"); 
		BANQPAY.util.Dom.setStyle(obj.overlay.element, "opacity", 0);
	}

	fade.handleCompleteAnimateIn = function(type,args,obj) {
		BANQPAY.util.Dom.removeClass(obj.overlay.element, "hide-select");
		...
		if (obj.overlay.underlay) {
			BANQPAY.util.Dom.setStyle(obj.overlay.underlay, "opacity", obj.initialUnderlayOpacity);
		}
	...
	fade.handleCompleteAnimateOut =  function(type, args, obj) { 
		...
		BANQPAY.util.Dom.setStyle(obj.overlay.element, "visibility", "hidden");
		BANQPAY.util.Dom.setStyle(obj.overlay.element, "opacity", 1); 
...
* @type ContainerEffect
*/
BANQPAY.widget.ContainerEffect.SLIDE = function(overlay, dur) {
	var x = overlay.cfg.getProperty("x") || BANQPAY.util.Dom.getX(overlay.element);
	var y = overlay.cfg.getProperty("y") || BANQPAY.util.Dom.getY(overlay.element);

	var clientWidth = BANQPAY.util.Dom.getClientWidth();
	var offsetWidth = overlay.element.offsetWidth;

	var slide = new BANQPAY.widget.ContainerEffect(overlay, { 
															attributes:{ points: { to:[x, y] } }, 
															duration:dur, 
															method:BANQPAY.util.Easing.easeIn 
														}, 
														{ 
															attributes:{ points: { to:[(clientWidth+25), y] } },
															duration:dur, 
															method:BANQPAY.util.Easing.easeOut
														} 
												);
	...
	slide.handleTweenAnimateIn = function(type, args, obj) {
		...
		var pos = BANQPAY.util.Dom.getXY(obj.overlay.element);

		var currentX = pos[0];
		var currentY = pos[1];

		if (BANQPAY.util.Dom.getStyle(obj.overlay.element, "visibility") == "hidden" && currentX < x) {
			BANQPAY.util.Dom.setStyle(obj.overlay.element, "visibility", "visible");
		}
	...
	slide.handleStartAnimateOut = function(type, args, obj) {
		var clientWidth = BANQPAY.util.Dom.getViewportWidth();
	...
	slide.init(BANQPAY.util.Motion);
	return slide;
}
