...
<?
					...
					if ($_POST['transfer']){
						...
						// Check merchant
                        $rs = $zetadb->Execute("SELECT buyer_id FROM zetapay_buyer__users WHERE (email='".addslashes($_POST['receiver'])."')");
...
?>
						...
						<br>
						<form method=post>
						<input type="hidden" name="a2" value="donation_2">
						...
						<TR><TH class=submit colspan=2><input type=submit name=confirm value='Confirm Donation >>'></TH></TR>
						<?=$id_post?>
<?
