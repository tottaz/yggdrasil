...
<?
					...
					if ($_POST['transfer']){
						...
						if (!$r){
							errform("There are no merchant with the specified Merchant IDusername", 'merchant_id');
							$posterr = 1;
						}
						...
						$from = $afrom->EMAIL;
						$username = $afrom->NAME;
						if($name == $_POST['receiver'] || $from == $_POST['receiver']){
							errform("You cannot send money to yourself", 'name');
						}
					}
