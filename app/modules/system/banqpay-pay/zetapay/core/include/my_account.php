...
<?
	...
	if ($base->input['buyer_edit'] && !$posterr){
		...
		/* No idea wat this code is doing probably signing up new user

		if ($base->input['email'] != $data->EMAIL){
			$uid = substr( md5(time()), 8, 16 );
			$zetadb->Execute("INSERT INTO cpos_buyer_signups SET
							id='$uid',
							email='{$base->input['email']}',
							expire=DATE_ADD(NOW(),INTERVAL 3 DAY)") or die( $zetadb->Error() );
			mail($base->input['email'], "Confirm E-mail for $sitename",
			$emailtop.gettemplate("email_edit", "$siteurl/zetapay/admin/confirmmail.php?id=$uid").$emailbottom,
			$defaultmail);
			prpage("html_edit");
		}else{
			// Go to account
			$action = 'buyer_account';
            $rs = $zetadb->Execute("SELECT * FROM cpos_buyer_users WHERE buyer_id=$user");
...
<script>
//	loadProvinces(document.getElementById('location-ltn_state_code'), Trim(document.getElementById('location-ltn_country_code').value));
//	document.getElementById('location-ltn_state_code').value = '<? echo $base->input['location-ltn_state_code']; ?>';
