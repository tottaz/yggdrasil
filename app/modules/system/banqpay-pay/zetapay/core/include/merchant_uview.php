...
if ($_GET['ed'] && $user == 3){
    ...
    $rs = $zetadb->Execute("SELECT * FROM zetapay_merchant_users WHERE merchant_id=".(int)$_GET['user']);
    $r = $rs->FetchNextObject();
	if ($r){
		$r->VIEW_COUNTER++;
		$zetadb->Execute("UPDATE zetapay_merchant_users SET view_counter={$r->VIEW_COUNTER} WHERE merchant_id={$r->merchant_id}");
...
<?
		echo 	"<TABLE class=design cellspacing=0 width=100%>\n",
				...
				($user == 3 ? " <a href=index.php?a=merchant_uview&user=$r->merchant_id&$id&ed=$r->merchant_id>Edit</a>" : ""),
				"<TR><TD class=row2 width=25%>E-mail:\n",
				"<TD class=row2>",($r->EMAIL ? htmlspecialchars($r->EMAIL) : '&nbsp;'),
				"<TR><TD class=row1>Company:\n",
