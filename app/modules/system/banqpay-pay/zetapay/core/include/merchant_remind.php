		...
		<tr>
			...
			<td width="519" valign="top">
				...
				<tr>
					<td bgcolor="#FFFFFF">
<?
	if ($_POST['merchant_remind']){
		$posterr = 0;
		// Check email
        $rs = $zetadb->Execute("SELECT * FROM zetapay_merchant_users WHERE email='".addslashes($_POST['email'])."'");
		$data = $rs->FetchNextObject();
		if (!$data)
			errform('There are no accounts registered with the email address you specified.');
	}
	if ($_POST['merchant_remind'] && !$posterr){
		wrapmail($_POST['email'], "$sitename Username and Password", $emailtop.gettemplate('merchant_email_remindpsw', '', $data->NAME, $data->PASSWORD).$emailbottom, $defaultmail);
		prpage('merchant_html_remindpsw');
	}else{
?>
						<BR>
						<CENTER>
						...
						<TR>
							<TH colspan=2 class=submit><INPUT type=submit class=button name=merchant_remind value='Submit >>'></TH>
							<?=$id_post?>
						</TR>
						</FORM>
						</TABLE>
