...
// Load Application level constants'
    include_once($rootDir.$subDir.'core/include/appconstants.php');
    
    require($rootDir.$subDir.'connect.php');// Include config data
    require($rootDir.$subDir.'sys-config1.php');
    require($rootDir.$subDir.'sys-config2.php');
//    require($rootDir.$subDir.'core/include/select_values.php');
    include_once($rootDir.$subDir.'core/adodb/adodb.inc.php');
    require($rootDir.$subDir.'core/include/ipsecure.php');	// Secure IP Stuff
	...
	function dpsumm2($summ, $design = 0){
		...
		if ($design){
			return "<span class=".
					($summ > 0 ? "minus>".($summ ? "-" : "").$currency : "plus>+$currency").
					number_format( ($summ > 0 ? $summ : -$summ), 2)."</span>";
		}else{
			return $currency.number_format($summ, 2, '.', '');
		}
	}
	...
	function dp($user, $uname = '', $sp = -1){
		...
		if (!$uname || $sp == -1){
            $rs = $zetadb->Execute("SELECT email, company FROM ".TBL_SYSTEM_USERS." WHERE id=$user");
            $r1 = $rs->FetchRow();
            $uname = $r1[0];
            $company = $r1[1];
		}
		...
		return "$company";
//		return "<a href=index.php?a=merchant_uview&user=$user&$id>$email</a>".$confirmed.$confirmed_cc.$confirmed_ph;
	}
