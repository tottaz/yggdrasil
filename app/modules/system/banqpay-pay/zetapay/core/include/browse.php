<?
	if($_GET['del']){
		$zetadb->Execute("DELETE FROM zetapay_shops WHERE id={$_GET['del']}");
	}
?>
...
<?
	...
	$sql = "SELECT * FROM zetapay_shops $where ORDER BY id $limit";
    $qr1 = $zetadb->Execute($sql);
	if($qr1 && ($qr1->RecordCount() > 0) ){
		...
		while (false!==($file = readdir($handle))) { 
			if ($file != "." && $file != "..") { 
				if( strstr($file , "x".$pid."_") ){
					...
					echo "<a href=\"{$row->url}\" target=\"_new\"><images src='$furl' border=0></a>";
				}
			} 
		}
		closedir($handle); 
...
<?
				if( ($user == $row->owner) || ($user == 3) ){
					echo "<a href=?a=merchant_submit_site&edit=$pid>edit shop</a> | ";
