...
<?
		...
		$balance = buyer_balance($user, 1);
		$rs = $zetadb->Execute("SELECT paidto,amount,comment,trdate,fees FROM zetapay_buyer_transactions WHERE (paidto=$user OR paidby=$user) AND pending=0 ORDER BY trdate DESC LIMIT 1");
...
?>
						...
						<tr bgcolor="#FFFFFF">
							...
							<td><?=$buyer_data->NAME?> <?=confirmStr($user)?></td>
						</tr>
						<tr bgcolor="#FFFFFF">
							<td height="20"><strong>Email:</strong></td>
							<td><?=$buyer_data->EMAIL?></td>
						</tr>
<?	if($allow_verify){	?>
						<tr bgcolor="#FFFFFF">
							<td height="20"><strong>Status:</strong></td>
							<td><?=verifiedBuyerLink($user)?></font></td>
						</tr>
<?	}	?>
						<tr bgcolor="#FFFFFF">
...
<?
					...
					$r2 = $zetadb->Execute("SELECT * FROM zetapay_buyer_transactions WHERE (paidto=$user OR paidby=$user) ORDER BY trdate DESC LIMIT 0,7");
					$i = 1;
...
?>
								<TR>
									...
									<TD class=row<?=$i?> style="<?=$style?>" align=center><?=dpbuyer($a->PAIDTO == $user ? $a->PAIDBY : $a->PAIDTO)?>
									...
									<TD class=row<?=$i?> style="<?=$style?>" align=center><a href="?a=buyer_transdet&did=<?=$a->ID?>">Details</a>
								</TR>
<?
						$i = 3 - $i;
					}
?>
								...
								<a href=?a=view_buyer_transactions&<?=$id?>>View All Transactions</a></SPAN>
								<br>
							</td>
						</tr>
						</table>
...
<script>
	loadDefaultCountry('country', 'state');
</script>