<?
	...
	if($allow_verify){
		...
		if ($veri < 1){
			...
			$qr1 = $zetadb->Execute("SELECT sum(amount) AS amount FROM zetapay_transactions WHERE (paidby=$user) AND (paidto<101) AND UNIX_TIMESTAMP(trdate)>$old_time AND UNIX_TIMESTAMP(trdate)<$time2 ORDER BY trdate") or die( mysql_error() );
			...
			if ($tot_w > $nv_limit){
				...
				echo ("<li>To become verified member - <a href=index.php?a=add_card&$id><b>Click here</b></a>");
				echo ("<br><br><br>");
...
<?       
		...
		case 4:
			...
			$rs = $zetadb->Execute("SELECT buyer_id FROM zetapay_users WHERE username='".addslashes($source)."'");
			$a = rs->FetchNextObject();
			if ($a){
				$addinfo = addslashes($_POST['addinfo']);
			}
			transact($user,$a->BUYER_ID,$amount,'Withdrawal','',$fee,1,$addinfo);
			$action = 'account';
			// Notify admin
			$message = $GLOBALS[$data->type]." $data->username has just withdrawn {$currency}$charge !";
			if ($wdr_notify){
