...
<?
	...
	if($source){
		...
		if ($paypal_use && $source == 'paypal'){
			include("core/payment/g_paypal.php");
		}elseif ($cc_use && $source == 'cc'){
			// 2CheckOut Credit card processing
			include("core/payment/g_2checkout.php");
		}elseif ($anet_use && $source == 'anet'){
			// Authorize.Net Credit card processing
			include("core/payment/g_anet.php");
		}elseif ($anet_use && $source == 'anetc'){
			// Authorize.Net Credit card processing
			include("core/payment/g_anetcheck.php");
		}elseif ($eg_use && $source == 'eg'){
			// E-Gold processing
			include("core/payment/g_egold.php");
		}elseif ($np_use && $source == 'np'){
			// Netpay processing
			include("core/payment/g_netpay.php");
		}elseif ($qchex_use && $source == 'qchex'){
			// Check processing
			include("core/payment/g_qchex.php");
		}elseif ($check_use && $source == 'check'){
			// Check processing
			include("core/payment/g_check.php");
		}
	}
	if (!$processed){
		// Generate form
