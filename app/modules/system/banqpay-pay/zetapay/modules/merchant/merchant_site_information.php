...
<?
	...
	if ($_POST['merchant_edit'] && !$posterr){
		...
		// Update database
		$query = "UPDATE ".TBL_SYSTEM_USERS." SET email='".addslashes($_POST['email'])."', name='".addslashes($_POST['name'])."', regnum='".addslashes($_POST['regnum'])."',notify={$_POST['notify']},address='".addslashes($_POST['address'])."',country='".addslashes($_POST['country'])."',state='".addslashes($_POST['state'])."',city='".addslashes($_POST['city'])."',phone1='".addslashes($_POST['phone'])."',fax='".addslashes($_POST['fax'])."',zipcode='".addslashes($_POST['zipcode'])."' WHERE id=$user";
		...
		if ($_POST['email'] != $merchant_data->EMAIL){
			...
			include('modules/merchant/my_account.php');
		}
	}else{
?>
<CENTER>
