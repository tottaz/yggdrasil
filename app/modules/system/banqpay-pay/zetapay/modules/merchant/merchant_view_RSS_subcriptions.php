<?
	...
	if ($_GET['d']){
	  $zetadb->Execute("DELETE FROM ".TBL_MERCHANT_RECUR." WHERE id='".(int)$_GET['d']."'");
    }
?>
				...
				<tr>
					<td>
						...
						<tr>
							...
							<th>&nbsp;
<?
                    		$rs = $zetadb->Execute("SELECT * FROM ".TBL_MERCHANT_RECUR." WHERE user=$user");
...
//								'1997-12-31 23:59:59'
								...
								$sqla .= " FROM ".TBL_MERCHANT_RECUR." WHERE id=".$a->ID;
...
?>
							<TR>
								...
								<TD class=row<?=$i?> align=center><a href=index.php?a=merchant_view_subscription&d=<?=$a->ID?>&<?=$id?>>Cancel</a>
							</TR>
<?
							$i = 3 - $i;
						}
				...
				<tr>
					<td>
						...
						<tr>
							...
							<th>&nbsp;
<?
                  		    $rs = $zetadb->Execute("SELECT * FROM ".TBL_MERCHANT_RECUR." WHERE receiver='".$data->EMAIL."'");
...
//								'1997-12-31 23:59:59'
                    		    ...
                    		    $rs = $zetadb->Execute("SELECT email FROM zetapay_users WHERE id=$a->USER");
...
?>
							<TR>
								...
								<TD class=row<?=$i?> align=center><?=dpdate3($d->NEXT);?>
								<TD class=row<?=$i?> align=center><?=dpdate3($k->EXPIRE);?>
								<TD class=row<?=$i?> align=center><a href=index.php?a=merchant_view_subscriptions&d=<?=$a->ID?>&<?=$id?>>Cancel</a>
							</TR>
<?
							$i = 3 - $i;
						}
