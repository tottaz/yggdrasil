...
<?
						...
						$qr2 = $zetadb->Execute("SELECT paidto,paidby,amount,addinfo,".TBL_USER_TRANSACTIONS.".comment AS comment,trdate,id,fees FROM ".TBL_USER_TRANSACTIONS." WHERE id='$did' ORDER BY trdate DESC");
						$numrows = $qr2->RecordCount();
						$r = $zetadb->Execute("SELECT paidto,paidby,amount,addinfo,".TBL_USER_TRANSACTIONS.".comment AS comment,trdate,id,fees FROM ".TBL_USER_TRANSACTIONS." WHERE id='$did' ORDER BY trdate DESC LIMIT $startp,$ulist_page");
						$i = 1;
					...
					#			$fremail = "";
							...
							echo
									...
									"<TR>",
									"	<TH>To</TH>",
									"	<TD class=row".($j+1)." >",dp($a->PAIDTO)," ",$toemail,
									"<TR>",
									"	<TH>From</TH>",
									"	<TD class=row".($j+1)." >",dp($a->PAIDBY)," ",$fremail,
									"<TR>",
									"	<TH>Total Amount</TH>",
