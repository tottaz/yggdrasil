...
<?
						...
						$qr2 = $zetadb->Execute("SELECT paidto,paidby,amount,addinfo,zetapay_buyer_transactions.comment AS comment,trdate,id,fees FROM zetapay_buyer_transactions WHERE id='$did' ORDER BY trdate DESC");
						$numrows = $qr2->RecordCount();
						$r = $zetadb->Execute("SELECT paidto,paidby,amount,addinfo,zetapay_buyer_transactions.comment AS comment,trdate,id,fees FROM zetapay_buyer_transactions WHERE id='$did' ORDER BY trdate DESC LIMIT $startp,$ulist_page");
						$i = 1;
						$current = buyer_balance($user, 1);
						$namount = 0;
						while ($a = $r->FetchNextObject()){
							$uto = dpbuyerObj($a->PAIDTO);
							$uby = dpbuyerObj($a->PAIDBY);
							$toemail = "(".$uto->EMAIL.")";
							$fremail = "(".$uby->EMAIL.")";
							if($a->PAIDBY != $user){
								$afrusr = dpbuyerObj($a->PAIDBY);
								if($a->FEES){
					...
					#			$fremail = "";
							...
							echo
									...
									"<TR>",
									"	<TH>To</TH>",
									"	<TD class=row".($j+1)." >",dpbuyer($a->PAIDTO)," ",$toemail,
									"<TR>",
									"	<TH>From</TH>",
									"	<TD class=row".($j+1)." >",dpbuyer($a->PAIDBY)," ",$fremail,
									"<TR>",
									"	<TH>Total Amount</TH>",
