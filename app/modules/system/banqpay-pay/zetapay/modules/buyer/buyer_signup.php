...
if ($_POST['buyer_signup']){
	...
	// Check if it exists
	if ($_POST['email']){
        $rs = $zetadb->Execute("SELECT buyer_id FROM zetapay_buyer_users WHERE email='".addslashes($_POST['email'])."'");
	...
	if(!$allow_same_ip){
		...
		if($ipaddress){
			$rs = $zetadb->Execute("SELECT ipaddress FROM zetapay_buyer_signups WHERE ipaddress='$ipaddress'");
            $qr = $rs->FetchNextObject();
			if ($qr) {
				errform('A user has already signed up from the same IP as you.', 'email'); // #err
			} else {
				$rs = $zetadb->Execute("SELECT ipaddress FROM zetapay_buyer_users WHERE (ipaddress='$ipaddress' OR lastip='$ipaddress')");
                $qr = $rs->FetchNextObject();
				if ($qr) {
					errform('A user has already signed up from the same IP as you.', 'email'); // #err
				}
...
if ($_POST['buyer_signup'] && !$posterr){
	...
	$rs = $zetadb->Execute("SELECT buyer_id FROM zetapay_buyer_users WHERE buyer_id='".addslashes($_POST['r'])."'");
    $r = $rs->FetchNextObject();
	$referred = ($r ? $r : "NULL");
...
#	$password = CreatePassword();
	$zetadb->Execute("INSERT INTO zetapay_buyer_signups SET
					id='$uid',
					email='{$_POST['email']}',
					pin='$pincode',
					password='$password',
	...
	wrapmail(	$_POST['email'], "Confirm E-mail for $sitename", $emailtop.gettemplate("buyer_email_signup", "$siteurl/zetapay/buyer/buyer_confirm.php?id=$uid",$info,$password).$emailbottom,$defaultmail);
  	prpage("buyer_html_signup");
} else {
	...
	<td>
		...
		<tr>
			...
			<td width="100%" valign="top">
				...
				<tr>
					<td>
						<span class="text4">Create&nbsp;a&nbsp;Buyer&nbsp;account</span><br>
...
<?	}	?>
										<TR>
											<TH colspan=2 class=submit>
												<INPUT type="submit" name=buyer_signup class=button value='Sign up >>'>
											</TH>
