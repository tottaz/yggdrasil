...
include("config.inc.php");
	...
	echo "All Used Accounts <br><br>";  // <--- just here as a lable for now
...
echo "<table border=1>";
	...
	printf('<input type="submit" name="sortname" class="topButton" value="Sort-Name">');	
    echo "</td></form>";
    echo "<td width=60>";
	printf('<form action="index.php?site=UsedCards"method="post">');
        printf('<input type="submit" name="buy" class="topButton" value="Value">');
    echo "</td></form>";
    echo "<td width=60>";
        printf('<form action="index.php?site=UsedCards"method="post">');
        printf('<input type="submit" name="used" class="topButton" value="Used">');
    echo "</td></form>";
    echo "</tr>";
...
//			AND 'SUM(AcctSessionTime)' >= '$Val'";
				...
				while($r2 = mysql_fetch_object($res2)) {
					...
					if(!isset($used)) { $used = -1; }  // These lines are to catch errors, NULLS and non numbers
					...
					if ($used >= $Val) {
						...
						printf('<input type="hidden" name="username" value="'); echo("$user"); printf('">');
        					printf('<input type="submit" name="user" class="smallButton" "value="'); 
						echo("$user");
...
if(isset($seluser)) {  // <---- If they selected a detailed view
	...
	while($r3 = mysql_fetch_object($res3)) {
		...
		$up = $r3->AcctOutputOctets;
			$up = ($up/ 1024); // kb
			$up = round($up);
		echo("<tr> <td>$start</td> <td>$stop</td> <td> $min </td> <td>$logout</td><td>$down</td> <td>$up</td></tr>");
		}
	echo('</table>');
