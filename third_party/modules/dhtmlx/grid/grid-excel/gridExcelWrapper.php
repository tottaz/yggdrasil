...
class gridExcelWrapper {
			...
			for ($i = 0; $i < count($columns); $i++) {
				for ($j = 0; $j < count($columns[$i]); $j++) {
					...
					if (isset($columns[$i][$j]['rowspan'])) {
						$rspan = min($columns[$i][$j]['rowspan'], count($columns) - $i);
						if ($rspan > 0)
							$this->excel->getActiveSheet()->mergeCells($this->getColName($j).($this->headerLinesNum + $i + 1).':'.$this->getColName($j).($this->headerLinesNum + $i + $rspan));
					}
				}
			}
			$styleArray = array(
    ...
    public function rowPrint($row, $rowHeight, $lineColor, $gridFontSize, $fontFamily, $indent) {
		...
		for ($i = 0; $i < count($row); $i++) {
			...
			$t = strtolower($this->types[$i]);
			if ($t == "edn" || $t == "ron" || $t == "number" || $t == "num") {
			...
			switch ($t) {
				...
				case 'date':
					...
					break;
				case 'link':
					$this->excel->getActiveSheet()->setCellValue($this->getColName($i).$this->currentRow, $text);
					$this->excel->getActiveSheet()->getCell($this->getColName($i).$this->currentRow)->getHyperlink()->setUrl($text);
					break;
				case 'tree':
					$this->excel->getActiveSheet()->getStyle($this->getColName($i).$this->currentRow)->getAlignment()->setIndent($indent*2);
				default:
					$this->excel->getActiveSheet()->setCellValueByColumnAndRow($i, $this->currentRow, $text);
					break;
			}
